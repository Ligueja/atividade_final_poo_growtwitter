...
export class User {
    ...
    public validateUsername(): void {
        const exist = users.some(user => user._username === this._username && user !== this);
    ...
    users.forEach(user => {
        console.log(`\n---------------------------\n`)
        console.log(`Nome: ${user.name}, Username: @${user.username}, Email: ${user.email}`);
    });         
    }

    // método para usuário disparar um tweet.
   public sendTweet(tweet: Tweet): void {
    ...
    public follow(user: User): void {
        ...
        this._following.push(user._username);
    }

    // metodo que mostra quais os usuários determinado usuário segue;
    public showFollowers(): void {
        console.log(`\n---------------------------\n`)
        console.log(`O usuário @${this._username} segue os seguintes usuários: @${this._following.join(', @')}`)
        console.log(`\n---------------------------\n`)
    }
